...
Given 3 variables "q" (for question), "t" (for when true) and "f" (for when false), the outcome of the operator is "[~]t" if "[~]q" evaluates to "true", 
...
| Infix         | a=0,b=0 | a=0,b=~0 | a=~0,b=0 | a=~0,b=~0 | Operator | Note
|:--------------|:-------:|:--------:|:--------:|:---------:|:--------:|:----:|
| a ?  false : b     | false | ~false |  false |  false | LT  | 1)
| a ? ~false : false | false |  false | ~false | ~false | a   | 2)
| a ? ~false : b     | false | ~false | ~false | ~false | OR  |
| a ?  b : false     | false |  false |  false | ~false | AND |
| a ? ~b : false     | false |  false | ~false |  false | GT  | 3)
| a ? ~b : b         | false | ~false | ~false |  false | NE  |
| a ?  b : c         |       |        |        |        | QTF | 4)
| a ? ~b : c         |       |        |        |        | QnTF| 4)

1) To avoid a non-inverted "when-true", LessThan is rewritten as not-GreaterThan by swapping inverted operands:
"~a ? false : ~b" -> "a ? ~b : false". 
Because of this it is implemented as part of normalisation and not part of the final collection.

2) This is implemented as part of normalisation and not part of the final collection.

3) Empirical evidence that logic operators are just as important as bitwise operators.

4) Empirical evidence based on generator metrics that OR/AND/XOR is a fraction of the power the unified operator makes available.
...
Example implementation of a `QnTF` only tree normalised for gcc.
